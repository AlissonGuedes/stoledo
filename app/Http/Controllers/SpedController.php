...
class SpedController extends Controller {
	...
	public function nao_escrituradas($cnpj, $data_inicio, $data_fim) {
		...
		if ( request() -> ajax()) {
			$dados['rows'] = $this -> sped_model -> getNFeNaoEscrituradas($cnpj, $data_inicio, $data_fim) -> get();
			$dados['recordsTotal'] = $this -> sped_model -> getNFeNaoEscrituradas($cnpj, $data_inicio, $data_fim, true) -> get() -> count();
			$dados['recordsFiltered'] = $this -> sped_model -> getNFeNaoEscrituradas(null, null, null, true) -> paginate(30);
	...
	public function baixar_xls($nfe) {
	...
	public function baixar_pdf($xml) {
		...
		// return view('relatorios.downloads.sped.nfe.pdf', $dados);
	}
